...
class DefaultStore<A : Action, E : Event, S : State>(
    ...
    private val interceptors = configuration.interceptors.toMutableList()
    ...
    init {
        ...
        interceptors.forEach { it.interceptState(initialState) }
        configuration.messageRelay?.subscribe { message ->
            configuration.messageHandler?.handle(
                ...
                interceptors.forEach { it.interceptMessage(message) }
            }
        }
    }
    ...
    override fun dispatch(action: A) {
        coroutineScope.launch {
            mutex.withLock {
                interceptors.forEach { it.interceptAction(action) }
                ...
                interceptors.forEach { it.interceptTransition(transition) }
                ...
                interceptors.forEach { it.interceptState(nextState) }
    ...
    private fun send(event: E) {
        interceptors.forEach { it.interceptEvent(event) }
    ...
    override fun add(interceptor: Interceptor<A, E, S>) {
